#!/usr/bin/env python3
"""RPTGS-TD3 trainer using the SB3-based PAVE/td3/models/rptgs_td3.py implementation.

Unlike train_rptgs.py / train_rptgs_silu.py (a standalone pure-PyTorch reference
implementation), this script exercises the actual RPTGSUnifiedTD3 class checked into the
...
same evaluation protocol as the other RPTGS variants.

Conditions matched to PAVE-TD3's own Pendulum settings:
  - SiLU activation (forced by RPTGSUnifiedTD3.__init__)
...
Deliberately kept at train_rptgs.py's value rather than SB3's 1e-3 default:
  - learning_rate=3e-4
"""
...
from td3.models.rptgs_unified_td3 import RPTGSUnifiedTD3  # noqa: E402
...
@th.no_grad()
def evaluate_policy(env_id: str, model: RPTGSUnifiedTD3, validation_seeds: list[int]) -> list[dict[str, object]]:
    env = gym.make(env_id)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eta", type=float, required=True,
                        help="RPTGS interpolation: 0=cap-only, 1=norm-balanced")
