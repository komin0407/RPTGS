#!/usr/bin/env python3
"""RPTGS-Cap-SAC (PAVE/sac/models/rptgs_cap_sac.py) on Reacher-v5.
...
identical to train_rptgs_sac_reacher.py.

Identical hyperparameters to the Pendulum SAC run (train_rptgs_sac.py): same
RPTGSCapSAC class (return-priority, cap-only two-task surgery on the actor;
...
from sac.models.rptgs_cap_sac import RPTGSCapSAC  # noqa: E402
...
@th.no_grad()
def evaluate_policy(model: RPTGSCapSAC, validation_seeds: list[int]) -> list[dict[str, object]]:
    env = gym.make(ENV_ID)
