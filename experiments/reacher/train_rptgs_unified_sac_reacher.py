#!/usr/bin/env python3
"""RPTGS-SAC (PAVE/sac/models/rptgs_sac.py) on Reacher-v5.

Identical hyperparameters to the Pendulum SAC run (train_rptgs_sac.py): same
RPTGSUnifiedSAC class (return-priority, norm-balanced two-task PCGrad on the actor;
...
for Pendulum, matching PAVE's Reacher schedule and the RPTGS-TD3 Reacher run).
"""
...
from sac.models.rptgs_unified_sac import RPTGSUnifiedSAC  # noqa: E402
...
@th.no_grad()
def evaluate_policy(model: RPTGSUnifiedSAC, validation_seeds: list[int]) -> list[dict[str, object]]:
    env = gym.make(ENV_ID)
